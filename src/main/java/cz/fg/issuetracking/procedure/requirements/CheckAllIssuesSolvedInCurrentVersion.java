...
/**
 * Check all issues solved
 *
 * @author Michal Franc, FG Forrest a.s. (c) 2013
 *         18.8.13 22:16
 */
public class CheckAllIssuesSolvedInCurrentVersion implements CheckProcedure {

    IssueManager issueManager;

    @Override
    public void check(ProcedureContext context) {
        List<Issue> issues = issueManager.getUnderDevelopmentIssues();
        if (issues.size()>0) {
            throw new IllegalStateException("There are unsolved issues for current version");
        }
    }
}
